...
public class WeatherStationSimulator extends Thread {
	...
	private WeatherDataVector weatherDataVector = new WeatherDataVector();
	...
	public WeatherDataVector generateWeatherDataVector() {
		...
		for (int i = 0; i < numOfReadings; i++) {
			...
			// logger.debug("Generating readings for "+fileName);
			WeatherDataReading _reading = new WeatherDataReading(windSpeed,
					...
					resolution);
			// logger.debug("Wind Speed "+ QoSMonitor.getInstance() +" " +
			// _reading.getSpeed() + " Temperature " + _reading.getTemperature()
			// +" size " +
			// _vector.getReadings().size() + 1 + " " +
			// qosMonitor.getCompressMgr().getCompression().name());
